...
Phase 1: Calculation
Goal: Calculate the sum of ASCII values for "IN".
Steps:
1. Use 'strings_to_chars_to_int' with the input string "IN".
2. Use 'add_list' to sum the resulting list of integers.
3. Once you have the final numerical sum, respond ONLY with a single line in the format:
   {final_answer_prefix} [number]
   Example: {final_answer_prefix} 157
4. DO NOT provide any other text or explanation in Phase 1.

Phase 2: Graphical Display (Starts ONLY after you provide FINAL_ANSWER)
Goal: Display the calculated number prefixed with "{text_prefix_in_paint}" in Paint.
Context: You have already calculated the final number in Phase 1. Let's call this `[final_number]`.
Steps: You MUST perform these actions sequentially, one FUNCTION_CALL per response line:
1. FUNCTION_CALL: open_paint
2. FUNCTION_CALL: draw_rectangle|{RECT_X1}|{RECT_Y1}|{RECT_X2}|{RECT_Y2}
3. FUNCTION_CALL: add_text_in_paint|{text_prefix_in_paint}[final_number]|{TEXT_CLICK_X}|{TEXT_CLICK_Y} (IMPORTANT: The first parameter MUST be the literal string "{text_prefix_in_paint}" followed by the actual number calculated in Phase 1. Click coordinates are {TEXT_CLICK_X}, {TEXT_CLICK_Y}.)

Example for Step 3 (if final number was 157):
FUNCTION_CALL: add_text_in_paint|FINAL ANSWER - 157|{TEXT_CLICK_X}|{TEXT_CLICK_Y}

IMPORTANT RULES:
- Respond with EXACTLY ONE line per turn: either a `FUNCTION_CALL:` or the `FINAL_ANSWER:`.
- Do not add explanations.
- Follow the Phase 2 steps in the exact order.
- Make sure the text parameter in the `add_text_in_paint` call includes the prefix "{text_prefix_in_paint}".
"""
